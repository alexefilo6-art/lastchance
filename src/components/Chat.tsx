...
import { Send, Bot, User, Sparkles, Loader2, X, ShieldAlert } from 'lucide-react';
...
export default function Chat({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      setMessages(msgs);
      if (msgs.length > 0) setError(null);
    }, (err) => {
      console.error("Firestore error in Chat:", err);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (openaiKey) {
        ...
        throw new Error('Nessuna chiave API trovata. Configura ANTHROPIC_API_KEY, OPENAI_API_KEY o GEMINI_API_KEY nei "Secrets".');
      }
      ...
      setError(err.message || "Si è verificato un errore durante la comunicazione con l'IA.");
    } finally {
      setIsLoading(false);
    }
  };
